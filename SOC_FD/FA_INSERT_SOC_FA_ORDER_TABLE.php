...
include 'conn.php';
	...
	if($_SERVER['REQUEST_METHOD'] === 'POST'){
		  ...
		  $P_ORDER_DEATILS=$_POST['P_ORDER_DEATILS'];
		  $P_ORDER_ADDRESS=$_POST['P_ORDER_ADDRESS'];
		  $P_ORDER_STATUS=$_POST['P_ORDER_STATUS'];
		  $P_CREATE_BY=$_POST['P_CREATE_BY'];
		 
		 $curs = oci_new_cursor($conn);

		 $REG = oci_parse($conn, 
		 "begin FOOD_DELIVERY_PROSESS_DATA.FA_INSERT_SOC_FA_ORDER_TABLE
		 (:CUR_DATA,
		 :P_USER_ID,
		 :P_PRODUCT_ID,
		 :P_ORDER_DEATILS,
		...
		oci_bind_by_name($REG, ":P_PRODUCT_ID", $P_PRODUCT_ID, -1, SQLT_CHR);
		oci_bind_by_name($REG, ":P_ORDER_DEATILS", $P_ORDER_DEATILS, -1, SQLT_CHR);
		oci_bind_by_name($REG, ":P_ORDER_ADDRESS", $P_ORDER_ADDRESS, -1, SQLT_CHR);
		oci_bind_by_name($REG, ":P_ORDER_STATUS", $P_ORDER_STATUS, -1, SQLT_CHR);
